...
public class H2SchemaInitializer {
    ...
    void initializeDedicatedSchemas() {
        try (Connection conn = dataSource.getConnection();
            ...
            log.info("Initialized H2 custom schemas with full table replication: {}", (Object) CUSTOM_SCHEMAS);
        } catch (Exception e) {
   
            // Fail loudly in test mode so we can see the actual error
            log.error("Failed to initialize custom H2 schemas: {}", e.getMessage(), e);
            throw new RuntimeException("H2 schema initialization failed - tests cannot proceed", e);
        }
    }
     ...
     * Note: H2 2.x requires GENERATED BY DEFAULT AS IDENTITY syntax instead of AUTO_INCREMENT.
     */
