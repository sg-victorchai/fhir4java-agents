...
```java
@Test
void webhookManagement_fullLifecycle() {
    // Register webhook
    WebhookRequest request = new WebhookRequest(
        "https://agent.example.com/callback",
        List.of("Patient.create", "Patient.update"),
    ...
    ResponseEntity<WebhookResponse> response = restTemplate.postForEntity(
        webhooksUrl, entity, WebhookResponse.class);
...
void sseStream_receivesEvents() {
    ...
    // Create resource and verify event received
}
```
...
void fullEventWorkflow() {
    ...
    // 3. Verify webhook is active and would receive event
}
```
...
### Task 17: BDD Tests for Phase 3 Events ✅

**Files:**
- Create: `fhir4java-server/src/test/resources/features/events/webhook-management.feature`
- Create: `fhir4java-server/src/test/resources/features/events/sse-streaming.feature`
- Create: `fhir4java-server/src/test/resources/features/events/resource-change-events.feature`
- Create: `fhir4java-server/src/test/resources/features/events/subscription-management.feature`
- Create: `fhir4java-server/src/test/java/org/fhirframework/server/bdd/steps/EventSteps.java`

- [x] **Step 1: Create webhook management feature file**
...
Feature: Webhook Management
  ...
  So that my agent can receive real-time notifications
...
- [x] **Step 2: Create SSE streaming feature file**
...
Feature: Server-Sent Events Streaming
  ...
  Scenario: Receive event when Patient is created
    ...
    Then I should receive an SSE event
    And the event type should be "resource-change"
```

- [x] **Step 3: Create resource change events feature file**
- [x] **Step 4: Create subscription management feature file**
- [x] **Step 5: Implement step definitions**
- [x] **Step 6: Run BDD tests**
- [x] **Step 7: Commit** `git commit -m "test: add BDD tests for Phase 3 real-time events"`

---

## AI Agent Integration Examples

The following tasks demonstrate how AI agents can integrate with the Phase 3 real-time events system.

### Task 18: AI Agent SSE Consumer Example ✅

**Files:**
- Create: `fhir4java-mcp/src/main/java/org/fhirframework/mcp/agent/AgentEventConsumer.java`
- Create: `fhir4java-mcp/src/test/java/org/fhirframework/mcp/agent/AgentEventConsumerTest.java`

Demonstrates how an AI agent can consume SSE events for real-time awareness.

- [x] **Step 1: Implement AgentEventConsumer**
...
public class AgentEventConsumer {
     ...
     * AI agents use this to receive real-time notifications.
     */
    public Flux<ResourceChangeEvent> subscribeToEvents(List<String> topics) {
        return webClient.get()
            .uri(uriBuilder -> uriBuilder
                .path("/api/events/stream")
                .queryParam("topics", String.join(",", topics))
                .build())
            .accept(MediaType.TEXT_EVENT_STREAM)
            .retrieve()
    ...
    /**
     * Example: Agent subscribes to Patient events for care coordination.
     */
    public void startPatientMonitoring() {
        subscribeToEvents(List.of("Patient", "Encounter", "Observation"))
            .subscribe(event -> {
                // Agent processes event - e.g., update context, trigger workflow
                log.info("Agent received: {} {} on {}",
...
- [x] **Step 2: Write tests for agent event consumer**
- [x] **Step 3: Run tests, verify pass**
- [x] **Step 4: Commit** `git commit -m "feat(mcp): add AI agent SSE consumer example"`

---

### Task 19: AI Agent Webhook Handler Example ✅

**Files:**
- Create: `fhir4java-mcp/src/main/java/org/fhirframework/mcp/agent/AgentWebhookHandler.java`
- Create: `fhir4java-mcp/src/test/java/org/fhirframework/mcp/agent/AgentWebhookHandlerTest.java`

Demonstrates how an AI agent can register webhooks and handle callbacks.

- [x] **Step 1: Implement AgentWebhookHandler**
...
public class AgentWebhookHandler {
     ...
     */
    public WebhookResponse registerAgentWebhook(
            String callbackUrl,
            List<String> topics,
            String hmacSecret) {
        ...
        return webClient.post()
            .uri("/api/webhooks")
            .header("X-Tenant-ID", agentContext.getTenantId())
            .bodyValue(request)
            .retrieve()
    ...
    /**
     * Handles incoming webhook callback.
     * Validates HMAC signature and processes event.
     */
    @PostMapping("/agent/webhook-callback")
    public ResponseEntity<Void> handleCallback(
            @RequestHeader("X-Webhook-Signature") String signature,
            @RequestBody ResourceChangeEvent event) {
    ...
    /**
     * Example: Agent reacts to new lab results.
     */
    private void handleCreate(ResourceChangeEvent event) {
        if ("Observation".equals(event.resourceType())) {
            // Trigger AI analysis workflow
            agentContext.analyzeNewLabResult(event.resourceId());
        }
    }
}
```

- [x] **Step 2: Implement HMAC signature validation**
- [x] **Step 3: Write tests for webhook handler**
- [x] **Step 4: Run tests, verify pass**
- [x] **Step 5: Commit** `git commit -m "feat(mcp): add AI agent webhook handler example"`

---

### Task 20: Event-Driven Agent Workflow Example ✅

**Files:**
- Create: `fhir4java-mcp/src/main/java/org/fhirframework/mcp/agent/workflow/EventDrivenWorkflow.java`
- Create: `fhir4java-mcp/src/test/java/org/fhirframework/mcp/agent/workflow/EventDrivenWorkflowTest.java`

Demonstrates a complete event-driven AI agent workflow combining MCP tools with real-time events.

- [x] **Step 1: Implement EventDrivenWorkflow**
...
public class EventDrivenWorkflow {
    ...
    /**
     * Complete workflow: Monitor patient → React to events → Update context
     *
     * Use Case: Clinical decision support agent that:
     * 1. Subscribes to lab result events
     * 2. Queries patient context when results arrive
     * 3. Generates alerts for critical values
     */
    public void startClinicalDecisionSupport(String patientId) {
        // Subscribe to Observation events for this patient
        eventConsumer.subscribeToEvents(List.of("Observation"))
            .filter(event -> isForPatient(event, patientId))
            .subscribe(event -> processLabResult(event, patientId));
    }

    private void processLabResult(ResourceChangeEvent event, String patientId) {
        // 1. Query the new observation
        ...
        // 2. Parse and analyze the result
        Observation observation = parseObservation(obsResponse);

        // 3. Check for critical values
        if (isCriticalValue(observation)) {
            // 4. Create an alert (using fhir_mutate)
            createCriticalAlert(patientId, observation);
        }

        // 5. Update agent context
        updatePatientContext(patientId, observation);
    }

    private void createCriticalAlert(String patientId, Observation obs) {
        // Use dry-run first to validate
        ToolCallResponse dryRun = mutateTool.execute(
            new ToolCallRequest("fhir_mutate", Map.of(
                "action", "create",
                "resourceType", "Flag",
                "dryRun", true,
                "body", buildCriticalFlag(patientId, obs)
            )));

        if (dryRun.isSuccess()) {
            // Create the actual alert
            mutateTool.execute(new ToolCallRequest("fhir_mutate", Map.of(
                "action", "create",
                "resourceType", "Flag",
                "body", buildCriticalFlag(patientId, obs)
            )));
...
- [x] **Step 3: Run tests, verify pass**
- [x] **Step 4: Commit** `git commit -m "feat(mcp): add event-driven agent workflow example"`

---

### Task 21: Agent Discovery of Event Capabilities ✅

**Files:**
- Modify: `fhir4java-mcp/src/main/java/org/fhirframework/mcp/tool/FhirDiscoverTool.java`
- Test: `fhir4java-mcp/src/test/java/org/fhirframework/mcp/tool/FhirDiscoverToolTest.java`

Extends fhir_discover to include event subscription capabilities in discovery response.

- [x] **Step 1: Add EVENT_CAPABILITIES topic to DiscoveryTopic**
...
public enum DiscoveryTopic {
    ...
    SEARCH_PARAMS,
    OPERATIONS,
    EVENT_CAPABILITIES,  // NEW: Discover SSE and webhook capabilities
    ALL
}
```

- [x] **Step 2: Implement event capability discovery**

```java
private DiscoveryResponse discoverEventCapabilities() {
    return new DiscoveryResponse(Map.of(
        "sse", Map.of(
            "endpoint", "/api/events/stream",
            "supportedFilters", List.of("topics", "actions"),
            "eventTypes", List.of("resource-change")
        ),
        "webhooks", Map.of(
            "endpoint", "/api/webhooks",
            "supportedTopicPatterns", List.of(
                "{ResourceType}.{action}",
                "{ResourceType}.*",
                "*.*"
            ),
        ...
        "subscriptions", Map.of(
            ...
            "channels", List.of("rest-hook", "sse")
        )
    ));
}
```

- [x] **Step 3: Write tests for event capability discovery**

```java
@Test
void fhirDiscover_eventCapabilities_returnsEventEndpoints() {
    ToolCallRequest request = new ToolCallRequest("fhir_discover",
        Map.of("topic", "EVENT_CAPABILITIES"));

    ToolCallResponse response = tool.execute(request);

    assertThat(response.getContent()).contains("sse");
    assertThat(response.getContent()).contains("/api/events/stream");
    assertThat(response.getContent()).contains("webhooks");
}
```

- [x] **Step 4: Run tests, verify pass**
- [x] **Step 5: Commit** `git commit -m "feat(mcp): add event capability discovery for AI agents"`
...
| Integration | 14, 16 | E2E MCP test, E2E Phase 3 test |
| Observability | 15 | MCP audit logging |
| Testing | 17 | BDD tests for Phase 3 events |
| AI Agent Integration | 18-21 | SSE consumer, webhook handler, event-driven workflow, capability discovery |
