...
CREATE TABLE IF NOT EXISTS fhir_search_index (
    ...
    CONSTRAINT uk_search_index UNIQUE (resource_id, param_name, param_type, composite_part,
        COALESCE(value_string, ''), COALESCE(value_token_system, ''), COALESCE(value_token_code, ''))
);
...
CREATE TABLE IF NOT EXISTS fhir_resource_tag (
    ...
    CONSTRAINT uk_resource_tag UNIQUE (resource_id, tag_type, COALESCE(system_uri, ''), COALESCE(code, ''))
);
...
DROP TRIGGER IF EXISTS trg_resource_last_updated ON fhir_resource;
...
GRANT EXECUTE ON ALL FUNCTIONS IN SCHEMA fhir TO fhir4java;

-- Set default schema for the user
ALTER ROLE fhir4java SET search_path TO fhir, public;

COMMENT ON SCHEMA fhir IS 'FHIR4Java - HL7 FHIR R5 resource storage schema';
COMMENT ON TABLE fhir_resource IS 'Primary storage for FHIR resources using JSONB';
COMMENT ON TABLE fhir_search_index IS 'Pre-computed search parameter indexes for efficient FHIR search';
